...
const Gauge_brix = ({ date, time_select, itemId }) => {
  ...
  const get_tem = async (api, id, start, end) => {
    let endpoints1 = api + api_path.brix + id + "/" + start + "/" + end;
    let endpoints2 = api + api_path.brixmin + id + "/" + start + "/" + end;
    let endpoints3 = api + api_path.brixavg + id + "/" + start + "/" + end;
    let endpoints4 = api + api_path.brixmax + id + "/" + start + "/" + end;
  ...
  return (
    <View style={styles.box}>
      ...
      >
        Brix : {temp.toFixed(2)} % ( {data_result_temp} )
      </Text>
